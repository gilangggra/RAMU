...
export function ActorDetailTabs({ actor, isCurrentActor }: ActorDetailTabsProps) {
  const [activeTab, setActiveTab] = useState<"about" | "portfolio" | "availability" | "reviews" | "needs">("about");
  ...
  const tabs = [
    ...
    },
    {
      ...
      label: isModel ? "Comp Card & Portofolio" : isStudio ? "Cyclorama & Peralatan" : isPhotographer ? "Peralatan & Galeri" : isDesigner ? "Keahlian & Karya" : "Portofolio & Aset",
      icon: <Sparkles className="w-4 h-4" />,
    },
    {
      id: "availability" as const,
      label: "Ketersediaan & Ketentuan",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="w-full bg-stone-100/80 p-1.5 rounded-[20px] flex items-center overflow-x-auto no-scrollbar shadow-inner border border-stone-200/50">
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex-1 shrink-0 px-4 py-2.5 rounded-2xl text-[13px] font-bold transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap ${
                isActive
                  ? "bg-white text-[#27213D] shadow-[0_2px_10px_rgba(39,33,61,0.06)] border border-stone-200/40"
                  : "bg-transparent text-stone-500 hover:text-[#27213D] hover:bg-stone-200/50 border border-transparent"
              }`}
            >
              <span className={`transition-colors duration-300 ${isActive ? "text-[#E66A48]" : "text-stone-400"}`}>
                {tab.icon}
              </span>
              <span>{tab.label}</span>
            </button>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="p-7 sm:p-8 rounded-[32px] bg-white/95 border border-stone-200/80 shadow-[0_4px_20px_rgba(39,33,61,0.03)] space-y-5">
                <div className="flex items-center gap-2 text-xs font-bold text-stone-400 uppercase tracking-wider">
                  <User className="w-3.5 h-3.5 text-[#E66A48]" />
                  <span>Bio & Filosofi Kreatif</span>
                </div>

                <div className="text-sm sm:text-base text-[#27213D] leading-relaxed font-normal">
                  {actor.description || "Profil kreatif terdaftar di ekosistem RAMU."}
                </div>

                {/* Role Specific Snapshot */}
                {isModel && modelAttrs && (
                  <div className="pt-4 border-t border-stone-100 grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="p-3 rounded-xl bg-stone-50 border border-stone-200/70">
                      <span className="text-[10px] font-bold text-stone-400 uppercase">Tinggi Badan</span>
                      <div className="text-base font-black text-[#27213D] mt-0.5">{modelAttrs.height_cm || 175} cm</div>
                    </div>
                    <div className="p-3 rounded-xl bg-stone-50 border border-stone-200/70">
                      <span className="text-[10px] font-bold text-stone-400 uppercase">B-W-H</span>
                      <div className="text-sm font-black text-[#27213D] mt-0.5">{modelAttrs.bust_waist_hips || "84-60-89"}</div>
                    </div>
                    <div className="p-3 rounded-xl bg-stone-50 border border-stone-200/70">
                      <span className="text-[10px] font-bold text-stone-400 uppercase">Sample Size</span>
                      <div className="text-sm font-black text-purple-700 mt-0.5">{modelAttrs.clothing_size || "S / 36 EU"}</div>
                    </div>
                    <div className="p-3 rounded-xl bg-stone-50 border border-stone-200/70">
                      <span className="text-[10px] font-bold text-stone-400 uppercase">Sepatu</span>
                      <div className="text-sm font-black text-[#27213D] mt-0.5">{modelAttrs.shoe_size || "39 EU"}</div>
                    </div>
                  </div>
                )}
      ...
      {activeTab === "portfolio" && (
        <div className="space-y-8">
          ...
          {/* All Registered Assets & Capabilities List */}
            ...
            <div className="flex items-center justify-between pb-3 border-b border-stone-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-extrabold text-[#27213D]">
                  Daftar Seluruh Aset & Kapabilitas Terdaftar ({actor.assets.length})
                </h3>
              </div>
